...
class PeptideHumanness:
    ...
    def is_human(self, min_fraction_subjects):
        assert (
            0 < min_fraction_subjects < 1
        ), "min_fraction_subjects should be between 0 and 1"
...
class ChainHumanness:
    ...
    def pos_imgt_mapping(self) -> Dict[Position, Position]:
        return {
            pos: imgt_pos
            for pos, imgt_pos in zip(self.chain.positions, self.imgt_chain.positions)
        }

    def get_oasis_curve(self, frequency=True, cumulative=True) -> pd.Series:
        bins = [
            get_fraction_subjects_bin(peptide.fraction_oas_subjects)
            for peptide in self.peptides.values()
        ]
        curve = (
            pd.Series(bins, name=self.chain.name)
            .value_counts()
            .reindex(FRACTION_SUBJECTS_BINS)
            .fillna(0)
        )
    ...
    def get_oasis_identity(self, min_fraction_subjects) -> float:
        return (
            self.get_num_human_peptides(min_fraction_subjects) / self.get_num_peptides()
        )
    ...
    def get_num_nonhuman_peptides(self, max_fraction_subjects) -> int:
        return sum(
            not p.is_human(max_fraction_subjects) for p in self.peptides.values()
        )
    ...
    def get_peptide_length(self) -> int:
        ...
        assert (
            len(set(lengths)) == 1
        ), f"Peptides should have same lengths, got: {set(lengths)}"
        return lengths[0]

    def get_positional_humanness(
        self, min_fraction_subjects
    ) -> List[Tuple[Position, str, List[PeptideHumanness]]]:
        ...
        for raw_pos, (pos, aa) in enumerate(self.chain):
            window = [
                chain_positions[i]
                for i in range(max(0, raw_pos - peptide_len + 1), raw_pos + 1)
            ]
            peptides = [
                self.peptides[peptide_pos]
                for peptide_pos in window
                if peptide_pos in self.peptides
            ]
            non_human_peptides = [
            ...
            [
                {
                    ...
                    if self.chain_type_residue_frequency.get(pos)
                    else None,
                    "Family Freq": self.germline_family_residue_frequency[pos].get(
                        aa, 0
                    )
                    if self.germline_family_residue_frequency.get(pos)
                    else None,
                }
    ...
    @classmethod
    def to_sequence_dataframe(
        cls, humanness_list: List["ChainHumanness"], species=True
    ) -> pd.DataFrame:
...
class AntibodyHumanness:
    ...
    def get_oasis_curve(self, frequency=True) -> pd.Series:
        if self.vh and self.vl:
            counts_curve = self.vh.get_oasis_curve(
                frequency=False, cumulative=False
    ...
    def get_num_peptides(self) -> int:
        return (self.vh.get_num_peptides() if self.vh else 0) + (
            self.vl.get_num_peptides() if self.vl else 0
        )

    def get_num_human_peptides(self, min_fraction_subjects) -> int:
        return (
            self.vh.get_num_human_peptides(min_fraction_subjects) if self.vh else 0
    ...
    def get_num_nonhuman_peptides(self, min_fraction_subjects) -> int:
        return (
            self.vh.get_num_nonhuman_peptides(min_fraction_subjects) if self.vh else 0
...
def chop_seq_peptides(seq: Union[SeqRecord, Chain], peptide_length):
    ...
    return [
        (pos, seq[i : i + peptide_length])
        for i, pos in enumerate(positions[: -peptide_length + 1])
    ]


def get_antibody_humanness(
    vh: Optional[Chain], vl: Optional[Chain], params: OASisParams
) -> AntibodyHumanness:
...
def get_chain_oasis_peptides(chain, params: OASisParams):
    ...
    if params.oasis_db_path.endswith(".gz"):
        raise ValueError(
            'The OASis DB file needs to be unzipped (use "gunzip DB_PATH.db.gz")'
        )

    if not os.path.exists(params.oasis_db_path):
        raise FileNotFoundError(
            f"The OASis DB path does not exist: {params.oasis_db_path}"
        )

    oas_engine = create_engine(
        "sqlite:///" + os.path.abspath(params.oasis_db_path), echo=False
    )

    oas_filter_chain = "Heavy" if chain.is_heavy_chain() else "Light"
    result = oas_engine.execute(
    ...
    return {
        pos: parse_peptide_humanness(
            pep,
            oas_hits=oas_grouped.get_group(pep)
            if pep in oas_grouped.groups
            else oas_hits[:0],
...
def get_chain_humanness(chain: Chain, params: OASisParams) -> ChainHumanness:
    ...
    imgt_chain = (
        chain
        if chain.scheme == "imgt" and chain.cdr_definition == "imgt"
        else chain.renumber("imgt")
    )
    ...
    num_germline_residues = sum(
        top_v.positions.get(pos) == aa or top_j.positions.get(pos) == aa
    ...
    return ChainHumanness(
        ...
        germline_family_residue_frequency=get_germline_family_residue_frequency(
            chain, imgt_chain, v_germline_family
        ),
        chain_type_residue_frequency=get_chain_type_residue_frequency(
            chain, imgt_chain
        ),
    )


def parse_peptide_humanness(
    peptide: str, oas_hits: pd.DataFrame, num_total_oas_subjects: int = None
) -> PeptideHumanness:
    return PeptideHumanness(
        seq=peptide,
        num_oas_subjects=len(oas_hits),
        fraction_oas_subjects=len(oas_hits) / num_total_oas_subjects
        if num_total_oas_subjects
        else None,
...
def get_fraction_subjects_bin(fraction):
    for bin_threshold, bin_label in zip(
        FRACTION_SUBJECTS_THRESHOLDS[::-1], FRACTION_SUBJECTS_BINS[::-1]
    ):
