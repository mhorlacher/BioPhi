...
@click.option(
    "--oasis-db", required=True, help="OAS peptide database connection string"
)
@click.option(
    "--scheme", default="kabat", help="Numbering scheme (kabat, chothia, imgt, aho)"
)
@click.option(
    "--cdr-definition",
    default="kabat",
    help="Numbering scheme (kabat, chothia, imgt, north)",
)
@click.option(
    "--min-percent-subjects",
    default=10,
    type=float,
    help="Minimum percent of OAS subjects to consider peptide human",
)
@click.option(
    "--summary",
    is_flag=True,
    default=False,
    type=bool,
    help="Only export a single summary sheet",
)
@click.option(
    "--summary-to-csv",
    is_flag=True,
...
def oasis(
   ...
   | |_| / ___ \___| | \__ \\
    ...
    assert (
        1 <= min_percent_subjects <= 90
    ), "--min-percent-subjects should be between 1 and 90"

    if not output.endswith(".xlsx") and not summary_to_csv:
        raise ValueError(
            f"The --output is a spreadsheet and should have an .xlsx extension"
        )
    ...
    (
        ...
        duplicate_names,
        unrecognized_files,
    ) = parse_antibody_files(files=inputs, verbose=True)

    if invalid_names:
        click.echo(
            f'Warning: Skipped {len(invalid_names):,} invalid inputs: {", ".join(invalid_names[:3])}',
            err=True,
        )

    if duplicate_names:
        click.echo(
            f'Warning: Skipped {len(duplicate_names):,} inputs with duplicate names: {", ".join(duplicate_names[:3])}',
            err=True,
        )
    ...
    oasis_params = OASisParams(
        oasis_db_path=oasis_db, min_fraction_subjects=min_percent_subjects / 100
    )
    pool = Pool()
    iterator = pool.imap(
        partial(
    ...
    results = list(
        tqdm(iterator, desc="Generating results", total=len(antibody_inputs))
    )
...
def humanness_task_wrapper(antibody_input, **kwargs):
    try:
        ...
        print(
            ...
            antibody_input.name,
            e,
            antibody_input.heavy_protein_seq,
            antibody_input.light_protein_seq,
        )
        return None


def show_unpaired_warning(antibody_inputs):
    has_both_chain_types = any(i.heavy_protein_seq for i in antibody_inputs) and any(
        i.light_protein_seq for i in antibody_inputs
    )
    num_unpaired = sum(
        (not i.heavy_protein_seq or not i.light_protein_seq) for i in antibody_inputs
    )

    if has_both_chain_types and num_unpaired:
        click.echo("Warning:", err=True)
        click.echo(
            f"Found {num_unpaired:,} unpaired "
            + ("chains" if num_unpaired > 1 else "chain"),
            err=True,
        )
        click.echo("These will be processed and analyzed separately")
        click.echo(
            "Please use the same fasta ID for heavy and light chain to report results for whole antibody"
            "(no spaces, optionally with _VH and _VL suffix).",
            err=True,
        )
        click.echo("", err=True)
