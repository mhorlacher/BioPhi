...
class HumanizeAntibodyTaskResult:
        ...
        return pd.Series(
            {
                ...
                (
                    "OASis Percentile",
                    "Before",
                ): self.parental_humanness.get_oasis_percentile(
                    self.oasis_params.min_fraction_subjects
                ),
                (
                    "OASis Percentile",
                    "After",
                ): self.humanized_humanness.get_oasis_percentile(
                    self.oasis_params.min_fraction_subjects
                ),
                (
                    "OASis Identity",
                    "Before",
                ): self.parental_humanness.get_oasis_identity(
                    self.oasis_params.min_fraction_subjects
                ),
                (
                    "OASis Identity",
                    "After",
                ): self.humanized_humanness.get_oasis_identity(
                    self.oasis_params.min_fraction_subjects
                ),
                (
                    "Non-human Peptides",
                    "Before",
                ): self.parental_humanness.get_num_nonhuman_peptides(
                    self.oasis_params.min_fraction_subjects
                ),
                (
                    "Non-human Peptides",
                    "After",
                ): self.humanized_humanness.get_num_nonhuman_peptides(
                    self.oasis_params.min_fraction_subjects
                ),
    ...
    def to_oasis_curve_dataframe(cls, results: List["HumanizeAntibodyTaskResult"]):
        ...
        df = pd.DataFrame([humanness.get_oasis_curve() for humanness in humanness_list])
        df["Antibody"] = index
        df["Type"] = types
    ...
    @classmethod
    def to_sheets(
        cls, results: List["HumanizeAntibodyTaskResult"], full=True
    ) -> Dict[str, pd.DataFrame]:
        sheets = {"Overview": cls.to_overview_dataframe(results)}
        if full:
            ...
            for result in results:
                ...
                assert len(peptides) == len(scores), (
                    "Scores and peptides not aligned in " + result.input.name
                )
    ...
    def get_humanized_records(self) -> List[SeqRecord]:
        ...
        if self.humanization.vh:
            records.append(
                self.humanization.vh.humanized_chain.to_seq_record(
                    description=f'VH ({self.get_export_name().replace("_"," ")})'
                )
            )
        if self.humanization.vl:
            records.append(
                self.humanization.vl.humanized_chain.to_seq_record(
    ...
    def get_parental_records(self) -> List[SeqRecord]:
        ...
        if self.humanization.vh:
            records.append(
                self.humanization.vh.parental_chain.to_seq_record(description="VH")
            )
        if self.humanization.vl:
            records.append(
                self.humanization.vl.humanized_chain.to_seq_record(description="VL")
            )
        return records


@celery.task()
def humanize_antibody_task(
    input: AntibodyInput,
    humanization_params: HumanizationParams,
    oasis_params: OASisParams,
) -> HumanizeAntibodyTaskResult:
    ...
    try:
        ...
        if vh_chain is not None and not vh_chain.is_heavy_chain():
            raise ValueError(
                f"Expected heavy chain, got {vh_chain.chain_type} chain: {vh_chain.seq}"
            )

        if vl_chain is not None and not vl_chain.is_light_chain():
            raise ValueError(
                f"Expected light chain, got {vl_chain.chain_type} chain: {vl_chain.seq}"
            )

        humanization = humanize_antibody(
            vh=vh_chain, vl=vl_chain, params=humanization_params
        )

        return HumanizeAntibodyTaskResult(
            humanization=humanization,
            parental_humanness=get_antibody_humanness(
                vh=vh_chain, vl=vl_chain, params=oasis_params
            ),
...
@celery.task()
def mutate_humanized_antibody_task(
    result: HumanizeAntibodyTaskResult, pos: str, aa: str
):
...
class HumannessTaskResult:
    ...
    def to_series(self):
        ...
        series = {
            ...
            "Threshold": threshold,
            "OASis Percentile": self.humanness.get_oasis_percentile(
                self.oasis_params.min_fraction_subjects
            ),
            "OASis Identity": self.humanness.get_oasis_identity(
                self.oasis_params.min_fraction_subjects
            ),
        ...
        if self.humanness.vh:
            ...
            series["Heavy OASis Percentile"] = self.humanness.vh.get_oasis_percentile(
                self.oasis_params.min_fraction_subjects
            )
            series["Heavy OASis Identity"] = self.humanness.vh.get_oasis_identity(
                self.oasis_params.min_fraction_subjects
            )
            series["Heavy Non-human peptides"] = (
                self.humanness.vh.get_num_nonhuman_peptides(
                    self.oasis_params.min_fraction_subjects
                ),
            )
        ...
        if self.humanness.vl:
            ...
            series["Light OASis Percentile"] = self.humanness.vl.get_oasis_percentile(
                self.oasis_params.min_fraction_subjects
            )
            series["Light OASis Identity"] = self.humanness.vl.get_oasis_identity(
                self.oasis_params.min_fraction_subjects
            )
            series[
                "Light Non-human peptides"
            ] = self.humanness.vl.get_num_nonhuman_peptides(
                self.oasis_params.min_fraction_subjects
            )
    ...
    @classmethod
    def to_oasis_curve_dataframe(
        cls, results: List["HumannessTaskResult"]
    ) -> pd.DataFrame:
        df = pd.DataFrame(
    ...
    def to_sheets(cls, results: List["HumannessTaskResult"], full=True):
        ...
        if full:
            sheets["VH"] = ChainHumanness.to_sequence_dataframe(
                [r.humanness.vh for r in results if r.humanness.vh], species=False
            )
            sheets["VL"] = ChainHumanness.to_sequence_dataframe(
                [r.humanness.vl for r in results if r.humanness.vl], species=False
            )
            sheets["OASis Curves"] = cls.to_oasis_curve_dataframe(results)
            for result in results:
